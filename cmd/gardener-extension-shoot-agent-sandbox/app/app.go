...
import (
	...
	configinstall "github.com/gardener/gardener-extension-shoot-agent-sandbox/pkg/apis/config/install"
	agentsandboxinstall "github.com/gardener/gardener-extension-shoot-agent-sandbox/pkg/apis/operator/install"
	"github.com/gardener/gardener-extension-shoot-agent-sandbox/pkg/controller/lifecycle"
)

// NewServiceControllerCommand creates a new command that is used to start the agent-sandbox controller.
func NewServiceControllerCommand() *cobra.Command {
	options := NewOptions()

	cmd := &cobra.Command{
		Use:           "gardener-extension-shoot-agent-sandbox",
		Short:         "Agent sandbox controller manages components which provide agent sandboxes in Gardener Shoot clusters.",
		SilenceErrors: true,
...
func (o *Options) run(ctx context.Context) error {
	...
	if err := agentsandboxinstall.AddToScheme(mgr.GetScheme()); err != nil {
		return fmt.Errorf("could not update manager scheme: %w", err)
	}

	if err := configinstall.AddToScheme(mgr.GetScheme()); err != nil {
		return fmt.Errorf("could not update manager scheme: %w", err)
	}
